...
workflow FastqToBam {
    ...
    scatter (sample in sampleConfig.samples) {
        ...
        scatter (rg in sample.readgroups) {
            ...
            call fastqc.FastQCPaired as fastqcPaired {
                input:
                    fastqcModule = fastqcModule,
                    inputFastq = getfastq1.link,
    ...
    output {
        ...
        Array [File] fastqcZip = flatten(fastqcPaired.zip)
        #cutadapt logs
        Array [File?] cutadaptLogs = select_all(flatten(cutadaptPe.fastq1Log))
        #markduplicates logs
        Array [File] markdupLogs = markDups.metrics
        Array [File] bai = if runBaseQualityRecalibration then select_all([applyBQSR.bai]) else prebqsrBai
        Array [File] bam = if runBaseQualityRecalibration then select_all([applyBQSR.bai]) else prebqsrBam
        Array [File] qcZip = bamQualityControl.qcZip
    }
